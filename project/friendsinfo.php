...
while($row = mysql_fetch_array($result))
  ...
  echo '<a href=http://localhost/PHP-Login/friendsinfo.php?id=' . $row["friends_id"] . '>' .'<font color="#0000FF">'. "Info" .'</font>' . '</a>';
...
while($row = mysql_fetch_array($result))
  ...
  echo '<a href=http://localhost/PHP-Login/friendsprofile1.php?id=' . $row["friends_id"] . '>' . "Photos" . '   ('. $numberOfRows . ')' . '</a>';
