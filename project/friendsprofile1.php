...
while($row = mysql_fetch_array($result))
  ...
  echo '<a href=http://localhost/PHP-Login/friendphoto.php?id=' . $row["friends_id"] . '>' . "Photos" . '   ('. $numberOfRows . ')' . '</a>';
...
while($row = mysql_fetch_array($result))
  ...
  echo '<div align="left">';
  echo '&nbsp;&nbsp;';
  echo '<a href=http://localhost/PHP-Login/member-index.php?id=' . $row["friends_id"] . '>' . $row['firstname'] . '</a>';
  echo '</div>';
  echo '</td>';
  echo '</tr>';
  echo '<tr>';
